...
int main()
{
	...
	/*for (;;)
	{
		double res = ComputeDistance();
		cout << res << endl;
	}*/
	
	
	/*FormateData(Point3i(0, 20000, 0), Speed(3000, 0, 0), g_buffer, ControlFlag::ABSOLUTE_POSITION);
	SendData(g_hCom, g_wrOverlapped, g_buffer, sizeof(g_buffer));
	Sleep(1000);
	for (int i = 0; i < 20; i++)
	{
		FormateData(Point3i(4000 * i, 20000, 0), Speed(4000, 0, 0), g_buffer, ControlFlag::ABSOLUTE_POSITION);
		SendData(g_hCom, g_wrOverlapped, g_buffer, sizeof(g_buffer));
		Sleep(1000);
	}*/
=======
...
>>>>>>> parent of 3a44e45... 完成单根焊条完全焊完的测试
